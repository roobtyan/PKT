...
class LightBackbone(nn.Module):
    def __init__(
        ...
        self.stem = nn.Sequential(
            ConvBnAct(in_channels, stem_channels, 3, stride=2, padding=1, act_layer=act_layer),
            ConvBnAct(stem_channels, stem_channels, 3, stride=1, padding=1, act_layer=act_layer),
        )

        self.stages = nn.ModuleList()
        stage_in = stem_channels
        for stage_channel, stride, block in zip(stage_channels, stage_strides, num_blocks):
